...
public class CustomCompactHttpLoggingMiddleware : DelegatingHandler
{
    ...
    public CustomCompactHttpLoggingMiddleware(ILogger<CustomCompactHttpLoggingMiddleware> logger, CompactHttpLoggingMiddlewareConfig config)
    {
    ...
    private async Task LogRequestResponse(HttpRequestMessage request, HttpResponseMessage response, long elapsedMilliseconds)
    {
        ...
        var logLevel = _config.LogLevelSelector(request.Method, request.RequestUri, response.StatusCode, responseBody.Length, null, elapsedMilliseconds);
        if (logLevel == LogLevel.None)
        {
            return;
        }
        _logger.Log(logLevel: logLevel, message: "Sent HTTP Request {Method} {Uri} {RequestBody}. Got response {HttpStatus} {ResponseBody}. Elapsed {ElapsedMilliseconds}", request.Method, request.RequestUri, _config.RequestBodyRedactor(requestBody), (int)response.StatusCode, _config.ResponseBodyRedactor(responseBody), elapsedMilliseconds);
    }
    private async Task LogRequestException(HttpRequestMessage request, Exception exception, long elapsedMilliseconds)
    {
        string? requestBody = request.Content is null ? null : await request.Content.ReadAsStringAsync().ConfigureAwait(false);
        var logLevel = _config.LogLevelSelector(request.Method, request.RequestUri, null, null, exception, elapsedMilliseconds);
        if (logLevel == LogLevel.None)
        {
            return;
        }
        _logger.Log(logLevel: logLevel, exception: exception, message: "Sent HTTP Request {Method} {Uri} {RequestBody}. Got error. Elapsed {ElapsedMilliseconds}", request.Method, request.RequestUri, _config.RequestBodyRedactor(requestBody), elapsedMilliseconds);
    }
}