...
namespace ToolsPack.Log4net
{
	public static class Log4NetQuickSetup
	{
		...
		public static void SetUpConsole(string pattern = DefaultPattern, string repository = null, Type repositoryType = null)
		{
            ...
            if (repository == null)
                repository = Guid.NewGuid().ToString();
            var repo = repositoryType == null ? LogManager.CreateRepository(repository) : LogManager.CreateRepository(repository, repositoryType);

            BasicConfigurator.Configure(repo, appender);
        }

		public static void SetUpFile(string filePath, string pattern = DefaultPattern, string repository=null, Type repositoryType=null)
		{
			var layout = new PatternLayout(pattern);
