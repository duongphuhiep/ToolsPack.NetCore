...
{
    private const string DefaultOutputTemplateConst =
    ...
    /// </summary>
    public static ILoggerFactory CreateLoggerFactory(this IMessageSink messageSink, MockLogger? mockLogger = null,
        string outputTemplate = DefaultOutputTemplateConst)
    {
    ...
    /// </summary>
    public static IServiceCollection AddTestLogging(this IServiceCollection service, IMessageSink messageSink,
        MockLogger? mockLogger = null, string outputTemplate = DefaultOutputTemplateConst)
    {
    ...
    private static Logger CreateSerilogLogger(this IMessageSink messageSink, string outputTemplate)
    {
    ...
    private static ILoggingBuilder BuildTestLogging(this ILoggingBuilder builder, Logger? serilogLogger,
    ...
    ///     - and to Seq localhost:5341 (only in debug mode).
    /// </summary>
    public static ILoggerFactory CreateLoggerFactory<T>(this IMessageSink messageSink, MockLogger<T>? mockLogger = null,
        string outputTemplate = DefaultOutputTemplateConst)
    {
        var serilogLogger = CreateSerilogLogger(messageSink, outputTemplate);
        return LoggerFactory.Create(builder => builder.BuildTestLogging(serilogLogger, mockLogger));
    ...
    ///     Inject TestOutput, Seq (debug only) and MockLogger (optional) to the application logging
    /// </summary>
    public static IServiceCollection AddTestLogging<T>(this IServiceCollection service, IMessageSink messageSink,
        MockLogger<T>? mockLogger = null, string outputTemplate = DefaultOutputTemplateConst)
    {
        var serilogLogger = CreateSerilogLogger(messageSink, outputTemplate);
        service.AddLogging(builder => builder.BuildTestLogging(serilogLogger, mockLogger));
        return service;
