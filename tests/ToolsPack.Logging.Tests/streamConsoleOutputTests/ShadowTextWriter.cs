...
{
    private static readonly object _lockMessageSink = new object();
    private static readonly object _lockTestOutput = new object();
...
public class ShadowTextWriter : TextWriter
{
    ...
    public override void Close() => shadown.Close();
    public override ValueTask DisposeAsync() => shadown.DisposeAsync();
    public override void Flush() => shadown.Flush();
    public override Task FlushAsync() => shadown.FlushAsync();
    public override Task FlushAsync(CancellationToken cancellationToken) => shadown.FlushAsync(cancellationToken);
